...
#   2. Der Arbeitsbaum wird VOR do_release auf Sauberkeit geprueft (do_release macht git add -A).
...
Z_SAUBER=$(zeile_in_lokal 'git status --porcelain')
...
pruefe "Arbeitsbaum-Pruefung vorhanden"   "ja" "$([ -n "${Z_SAUBER:-}" ] && echo ja || echo nein)"
...
if [ -n "${Z_SAUBER:-}" ] && [ -n "${Z_NAS:-}" ] && [ -n "${Z_SUFFIX:-}" ] && [ -n "${Z_RELEASE:-}" ] && [ -n "${Z_PROD:-}" ]; then
    pruefe "Arbeitsbaum-Pruefung VOR do_release" "ja" "$([ "$Z_SAUBER" -lt "$Z_RELEASE" ] && echo ja || echo nein)"
...
# 5. Rueckbau nimmt nur zurueck, was NICHT auf origin ist
